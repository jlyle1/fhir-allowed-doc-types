# CDS Hooks - Allowed Document Types Operation v1.0.0
...
1. Application invokes`document-select`hook
1. CDS Service returns available document types as selectable suggestions
1. User selects the appropriate document type
1. Application creates the document with the selected type
...
The CDS Service implementing this hook may call the [$allowed-document-types](operations.md) operation internally to determine available document types, then transform the response into CDS Hooks cards.
